...
import { Topping, UpdateToppingDto } from '@/lib/types/topping.types';
import { useAppSelector, useAppDispatch } from '../../lib/hooks/redux';
import Input from '../ui/Input';
import Alert from '../ui/Alert';
import { updateTopping, clearError } from '@/lib/redux/slices/toppingsSlice';

interface EditToppingFormProps {
  topping: Topping;
  onSuccess: () => void;
  onCancel: () => void;
}

export default function EditToppingForm({ topping, onSuccess, onCancel }: EditToppingFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    maximumAmount: topping.maximumAmount?.toString() || '1',
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (formData.name.trim() !== topping.name) {
      updateData.name = formData.name.trim();
    }

    if (Number(formData.price) !== topping.price) {
      updateData.price = Number(formData.price);
    }

    if (Number(formData.maximumAmount) !== topping.maximumAmount) {
      updateData.maximumAmount = Number(formData.maximumAmount);
    }

    if (formData.isActive !== topping.isActive) {
      updateData.isActive = formData.isActive;
    }

    try {
      await dispatch(updateTopping({
        name: topping.name,
        updateData
      })).unwrap();
      onSuccess();
    } catch (error) {
      console.error('Update topping failed:', error);
    }
  };
